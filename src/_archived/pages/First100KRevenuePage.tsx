...
import { useFirst100KRevenue } from '@/hooks/useFirst100KRevenue';
import {
  TrendingUp, DollarSign, Megaphone, BarChart3, ArrowRight, Target,
  CheckCircle2, Zap, Shield, Layers, Rocket,
} from 'lucide-react';

const catMeta: Record<string, { label: string; icon: React.ReactNode; color: string }> = {
  deals: { label: 'High-Value Deal Concentration', icon: <Target className="w-3.5 h-3.5" />, color: 'text-primary' },
  monetization: { label: 'Monetization Activation', icon: <DollarSign className="w-3.5 h-3.5" />, color: 'text-primary' },
  demand: { label: 'Demand Generation', icon: <Megaphone className="w-3.5 h-3.5" />, color: 'text-primary' },
  consistency: { label: 'Revenue Consistency', icon: <BarChart3 className="w-3.5 h-3.5" />, color: 'text-primary' },
};
...
export default function First100KRevenuePage() {
  const { modules, leversByCategory, weeklyKPIs, monetizationItems, scalingValidators, milestonePhases, categories } = useFirst100KRevenue();
      ...
      <div className="border-b border-border bg-card/60 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <h1 className="text-xl font-bold text-foreground tracking-tight flex items-center gap-2">
            ...
            First $100K Monthly Revenue Execution
          </h1>
          <p className="text-xs text-muted-foreground mt-0.5">Tactical framework to achieve Rp 1.5B+ ($100K) monthly revenue through deal closing, monetization activation, and growth discipline</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-4">
        <Tabs defaultValue="roadmap" className="space-y-4">
          <TabsList className="grid grid-cols-2 md:grid-cols-5 w-full">
            ...
            <TabsTrigger value="modules" className="text-xs">Revenue Modules</TabsTrigger>
            <TabsTrigger value="weekly" className="text-xs">Weekly KPIs</TabsTrigger>
            <TabsTrigger value="monetization" className="text-xs">Monetization Priority</TabsTrigger>
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <Badge className="text-[9px] bg-emerald-500/15 text-emerald-400">{phase.revenueTarget}</Badge>
                        </div>
                      </div>
                    </div>
          ...
          <TabsContent value="modules" className="space-y-4">
            {categories.map(cat => (
              <div key={cat} className="space-y-2">
                ...
                {leversByCategory(cat).map((mod, i) => (
                  <motion.div key={mod.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                    <Card className="border-border bg-card">
                      <CardContent className="p-3">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-[11px] font-medium text-foreground">{mod.action}</span>
                          <div className="flex items-center gap-1.5 shrink-0">
                            <Badge className={`text-[9px] ${effortBadge[mod.effort]}`}>{mod.effort}</Badge>
                            <Badge variant="outline" className="text-[9px]">{mod.timeline}</Badge>
                          </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                          <div className="rounded border border-border bg-primary/5 p-2">
                            ...
                            <span className="text-[11px] text-primary font-medium">{mod.impact}</span>
                          </div>
                          <div className="rounded border border-border bg-muted/10 p-2">
                            ...
                            <span className="text-[11px] text-foreground">{mod.kpi}</span>
                          </div>
                          <div className="rounded border border-border bg-emerald-500/5 p-2">
                            <span className="text-[10px] text-muted-foreground block">Revenue Contribution</span>
                            <span className="text-[11px] text-emerald-400 font-medium">{mod.revenueContribution}</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-3">
                      ...
                      <div className="rounded-lg border border-border bg-primary/5 p-2 text-center">
                        <span className="text-[10px] text-muted-foreground block">Deals Target</span>
                        <span className="text-xs font-bold text-primary">{week.dealsTarget}</span>
                      </div>
                      <div className="rounded-lg border border-border bg-blue-500/5 p-2 text-center">
                        <span className="text-[10px] text-muted-foreground block">Subscriptions</span>
                        <span className="text-xs font-bold text-blue-400">{week.subscriptionTarget}</span>
                      </div>
                    </div>
            ...
            <Card className="border-border bg-card">
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-bold text-foreground">Monetization Activation Priority Checklist</span>
                </div>
                <p className="text-[11px] text-muted-foreground mb-4">Ranked by revenue potential and activation readiness. Critical streams must be live by Week 2.</p>
                <div className="space-y-3">
                  {monetizationItems.map((item, i) => (
                    <motion.div key={i} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}>
                      <div className="rounded-lg border border-border bg-muted/5 p-3">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-xs font-medium text-foreground">{item.stream}</span>
                          <div className="flex items-center gap-1.5">
                            <Badge className={`text-[9px] ${priorityBadge[item.priority]}`}>{item.priority}</Badge>
                            <Badge className="text-[9px] bg-emerald-500/15 text-emerald-400">{item.monthlyPotential}</Badge>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-[10px] text-muted-foreground">Readiness</span>
                          <Progress value={item.readiness} className="flex-1 h-1.5" />
                          <span className="text-[10px] text-primary font-medium">{item.readiness}%</span>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          <div className="rounded border border-border bg-muted/10 p-2">
                            <span className="text-[10px] text-muted-foreground block">Status</span>
                            <span className="text-[11px] text-foreground">{item.currentStatus}</span>
                          </div>
                          <div className="rounded border border-border bg-primary/5 p-2">
                            <span className="text-[10px] text-muted-foreground block">Activation Action</span>
                            <span className="text-[11px] text-primary">{item.activationAction}</span>
                          </div>
                        </div>
                      </div>
                    </motion.div>
            ...
            <Card className="border-border bg-card">
              <CardContent className="p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-sm font-bold text-foreground">Scaling Readiness Validation Indicators</span>
                </div>
                <p className="text-[11px] text-muted-foreground mb-4">These signals must trend green before committing to aggressive expansion after hitting $100K monthly revenue.</p>
                <div className="space-y-3">
                  {scalingValidators.map((ind, i) => (
