      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <Badge variant="destructive" className="animate-pulse">
              {t('upgradeTriggers.inquiryExpiring.urgency')}
            </Badge>
          </div>
          <DialogTitle className="text-xl mt-3">{t('upgradeTriggers.inquiryExpiring.title')}</DialogTitle>
          <DialogDescription className="text-base">
            <span className="text-2xl font-bold text-primary">{inquiryCount}</span> {t('upgradeTriggers.inquiryExpiring.subtitle')}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-4">
          <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4 text-center">
            <p className="text-xs text-muted-foreground mb-1">{t('upgradeTriggers.inquiryExpiring.warning')}</p>
            <div className="flex items-center justify-center gap-2">
              <Clock className={`h-6 w-6 ${urgencyColor}`} />
              <span className={`text-3xl font-bold ${urgencyColor}`}>{hoursLeft}</span>
              <span className={`text-lg ${urgencyColor}`}>{t('upgradeTriggers.inquiryExpiring.hoursLabel')}</span>
            </div>
          </div>
          ...
          <p className="text-sm text-destructive font-medium text-center">
            💸 {t('upgradeTriggers.inquiryExpiring.missedOpportunity')}
          </p>

          <div className="space-y-2 bg-primary/5 rounded-lg p-3 border border-primary/10">
            <h4 className="text-sm font-medium flex items-center gap-2">
              <Crown className="h-4 w-4 text-primary" />
              {t('upgradeTriggers.inquiryExpiring.proFeatures')}
            </h4>
            <ul className="space-y-1.5 text-sm">
              <li className="flex items-center gap-2">
                <Zap className="h-3.5 w-3.5 text-primary" />
                {t('upgradeTriggers.inquiryExpiring.benefit1')}
              </li>
              <li className="flex items-center gap-2">
                <Clock className="h-3.5 w-3.5 text-primary" />
                {t('upgradeTriggers.inquiryExpiring.benefit2')}
              </li>
              <li className="flex items-center gap-2">
                <MessageSquare className="h-3.5 w-3.5 text-primary" />
                {t('upgradeTriggers.inquiryExpiring.benefit3')}
              </li>
            </ul>
          </div>
        </div>
        ...
        <DialogFooter className="flex-col sm:flex-row gap-2">
          <Button variant="ghost" onClick={onClose} className="w-full sm:w-auto text-muted-foreground text-xs">
            {t('upgradeTriggers.inquiryExpiring.maybeLater')}
          </Button>
          <Button 
          ...
          >
            ...
            {t('upgradeTriggers.inquiryExpiring.upgradeNow')}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
