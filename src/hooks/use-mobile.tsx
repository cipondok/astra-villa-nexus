...
export function useIsMobile() {
  ...
  // Keep latest state in refs so the resize handler always compares against fresh values
  ...
  React.useEffect(() => {
    const checkMobile = () => {
      ...
      // Compare with latest values (refs) to avoid stale-closure bugs
      if (newIsMobile !== isMobileRef.current || newIsTablet !== isTabletRef.current) {
        ...
        if (newIsMobile) {
          ...
          bodyEl.classList.remove('mobile-device', 'mobile-app-layout', 'tablet-device', 'tablet-app-layout')
        }
