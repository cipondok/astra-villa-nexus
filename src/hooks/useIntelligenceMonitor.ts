...
// Use untyped client to avoid deep type instantiation
...
export function useIntelligenceMonitor() {
  return useQuery<IntelligenceMonitorData, Error>({
    ...
    queryFn: async (): Promise<IntelligenceMonitorData> => {
      ...
      const [totalProps, scoredProps, eliteProps, cacheData] = await Promise.all([
        ...
        db.from('ai_intelligence_cache').select('id', { count: 'exact', head: true }).eq('intelligence_type', 'opportunity_score'),
        db.from('ai_intelligence_cache').select('property_id, data, updated_at').eq('intelligence_type', 'opportunity_score').gte('data->>score', '85').order('updated_at', { ascending: false }).limit(10),
        db.from('ai_intelligence_cache').select('data, updated_at').eq('intelligence_type', 'opportunity_score').order('updated_at', { ascending: false }).limit(200),
      ]);
      ...
      if (elitePropertyIds.length > 0) {
        const { data: props } = await db.from('properties').select('id, title, city').in('id', elitePropertyIds);
      ...
      const [dailySessions, weeklySessions, searchCount, behaviorLogs] = await Promise.all([
        db.from('activity_logs').select('id', { count: 'exact', head: true }).gte('created_at', oneDayAgo) as any,
        db.from('activity_logs').select('id', { count: 'exact', head: true }).gte('created_at', sevenDaysAgo) as any,
        db.from('ai_property_queries').select('id', { count: 'exact', head: true }).gte('created_at', sevenDaysAgo) as any,
        db.from('activity_logs').select('activity_type, created_at').gte('created_at', sevenDaysAgo).order('created_at', { ascending: false }).limit(500) as any,
      ]);
