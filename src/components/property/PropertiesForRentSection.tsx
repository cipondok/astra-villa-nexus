    ...
    return (
      <section className="glass-card rounded-lg sm:rounded-xl md:rounded-2xl p-1.5 sm:p-2 md:p-3 border border-white/20 dark:border-white/10 bg-gradient-to-br from-white/80 via-white/60 to-white/40 dark:from-gray-900/80 dark:via-gray-900/60 dark:to-gray-900/40 backdrop-blur-xl shadow-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-1.5 sm:gap-2 md:gap-3">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="animate-pulse rounded-lg sm:rounded-xl overflow-hidden bg-white/50 dark:bg-white/5 h-28 sm:h-32 md:h-44"></div>
          ))}
        </div>
      </section>
    );
    ...
    return (
      <section className="glass-card rounded-lg sm:rounded-xl md:rounded-2xl p-1.5 sm:p-2 md:p-3 border border-white/20 dark:border-white/10 bg-gradient-to-br from-white/80 via-white/60 to-white/40 dark:from-gray-900/80 dark:via-gray-900/60 dark:to-gray-900/40 backdrop-blur-xl shadow-lg">
        ...
        <div className="text-center text-muted-foreground py-2 sm:py-3 text-[7px] sm:text-[8px] md:text-xs">
          Tidak ada properti disewa saat ini.
        </div>
      </section>
    );
  }
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-1.5 sm:gap-2 md:gap-3">
        {rentProperties.slice(0, 6).map((property) => (
          <div
            ...
            className="group cursor-pointer relative rounded-lg sm:rounded-xl overflow-hidden h-28 sm:h-32 md:h-44 hover:scale-[1.02] transition-all duration-200 ring-1 ring-blue-200/50 dark:ring-blue-800/30"
          >
            ...
            <div className="absolute top-1 left-1 right-1 sm:top-1.5 sm:left-1.5 sm:right-1.5 md:top-2 md:left-2 md:right-2 flex items-start justify-between">
              {/* Rent Badge */}
              <span className="text-[7px] sm:text-[8px] md:text-xs font-bold px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full bg-gradient-to-r from-blue-500 to-sky-600 text-white shadow-lg">
                Sewa
              </span>
              {/* Property Type */}
              <span className="text-[6px] sm:text-[7px] md:text-[10px] font-semibold px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full bg-white/95 dark:bg-black/80 text-foreground shadow-lg truncate max-w-[50%]">
            ...
            <div className="absolute top-1/2 left-1 sm:left-1.5 md:left-2 -translate-y-1/2">
              <span className="text-[8px] sm:text-[10px] md:text-sm font-bold px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-md bg-gradient-to-r from-blue-600 to-sky-700 text-white shadow-xl">
                {property.price >= 1000000000 
                  ? `${(property.price / 1000000000).toFixed(1)}M` 
                  : `${(property.price / 1000000).toFixed(0)}Jt`}
                <span className="text-[6px] sm:text-[8px] md:text-[10px] font-normal">/bln</span>
              </span>
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-1.5 sm:p-2 md:p-3">
              {/* Title */}
              <h3 className="text-[8px] sm:text-[10px] md:text-sm font-bold text-white line-clamp-1 drop-shadow-lg">
                {property.title}
              </h3>
              {/* Location */}
              <p className="text-[7px] sm:text-[8px] md:text-xs text-white/95 truncate drop-shadow-md mt-0.5">
                📍 {property.city || property.location}
              </p>
              {/* Property Details */}
              <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 mt-0.5 sm:mt-1 text-[7px] sm:text-[8px] md:text-[11px] text-white/90 font-medium">
                {property.bedrooms && (
