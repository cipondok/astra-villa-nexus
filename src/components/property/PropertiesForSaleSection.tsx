...
const PropertiesForSaleSection = ({ language, onPropertyClick }: PropertiesForSaleSectionProps) => {
  ...
  return (
    <section className="glass-card rounded-xl md:rounded-2xl p-2 md:p-3 border border-white/20 dark:border-white/10 bg-gradient-to-br from-emerald-50/80 via-white/60 to-green-50/40 dark:from-emerald-950/40 dark:via-gray-900/60 dark:to-green-950/30 backdrop-blur-xl shadow-lg">
      <div className="mb-1.5 md:mb-2 flex items-center justify-center gap-1.5 md:gap-2">
        <Home className="h-3 w-3 md:h-4 md:w-4 text-green-600 dark:text-green-400" />
        <h2 className="text-[10px] md:text-xs font-semibold text-foreground">
          Properties for Sale
        </h2>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 md:gap-3">
        {saleProperties.slice(0, 6).map((property) => (
          <div
            ...
            className="group cursor-pointer relative rounded-xl overflow-hidden h-36 md:h-44 hover:scale-[1.02] transition-all duration-200 ring-1 ring-green-200/50 dark:ring-green-800/30"
          >
            ...
            <div className="absolute top-1.5 left-1.5 right-1.5 md:top-2 md:left-2 md:right-2 flex items-start justify-between">
              {/* Sale Badge */}
              <span className="text-[9px] md:text-xs font-bold px-2 py-1 rounded-full bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-lg">
                Jual
              </span>
              {/* Property Type */}
            ...
            <div className="absolute top-1/2 left-1.5 md:left-2 -translate-y-1/2">
              <span className="text-xs md:text-sm font-bold px-2 py-1 rounded-md bg-gradient-to-r from-green-600 to-emerald-700 text-white shadow-xl">
