...
const LoadingPage: React.FC<LoadingPageProps> = ({ 
  ...
  const getConnectionInfo = () => {
    if (speed === 'slow') {
      return { message: t('loading.slowInternet'), color: 'text-chart-3', bgColor: 'bg-chart-3', Icon: AlertCircle };
    }
    if (speed === 'offline') {
      return { message: t('loading.noInternet'), color: 'text-destructive', bgColor: 'bg-destructive', Icon: WifiOff };
    }

    switch (connectionStatus) {
      case 'connecting':
        return { message: t('loading.checkingDatabase'), color: 'text-chart-3', bgColor: 'bg-chart-3', Icon: Wifi };
      case 'connected':
        return { message: t('loading.databaseReady'), color: 'text-chart-1', bgColor: 'bg-chart-1', Icon: CheckCircle };
      case 'error':
        return { message: t('loading.databaseUnavailable'), color: 'text-destructive', bgColor: 'bg-destructive', Icon: AlertCircle };
      case 'offline':
        return { message: t('loading.workingOffline'), color: 'text-muted-foreground', bgColor: 'bg-muted-foreground', Icon: WifiOff };
      default:
        return { message: t('loading.initializing'), color: 'text-muted-foreground', bgColor: 'bg-muted-foreground', Icon: Loader2 };
    }
  };
