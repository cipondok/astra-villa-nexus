...
import { Search, MapPin, Home, Bed, Bath, Car, Wifi, Shield, Activity, Crown, Sparkles, Zap, ArrowRight, Star, TrendingUp, BarChart3 } from "lucide-react";
...
const SearchFilters = ({ language, onSearch }: SearchFiltersProps) => {
  ...
  const amenityOptions = [
    ...
    { icon: Activity, key: "gym", label: language === "en" ? "Fitness Center" : "Pusat Kebugaran" }
  ];
  ...
  return (
    <Card className="astra-card border-0 shadow-2xl animate-astra-scale-in">
      <CardContent className="p-12">
        {/* ASTRA Header */}
        <div className="text-center mb-12">
          <div className="flex items-center justify-center gap-4 mb-6">
            <div className="p-4 rounded-2xl bg-gradient-to-r from-purple-500 via-blue-500 to-cyan-500 text-white animate-astra-glow">
              <BarChart3 className="h-8 w-8 animate-astra-float" />
            </div>
            <h2 className="text-4xl font-bold text-gradient-astra">ASTRA Property Search</h2>
          </div>
          <p className="text-muted-foreground text-xl">Advanced AI-powered property discovery platform</p>
        </div>

        {/* ASTRA Main Search Bar */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-12">
          <div className="lg:col-span-2">
            <div className="relative group">
              <Search className="absolute left-6 top-6 h-6 w-6 text-muted-foreground group-hover:text-purple-400 transition-colors" />
              <Input
                placeholder={currentText.search}
                className="input-astra pl-16 h-16 text-lg font-medium"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
              <div className="absolute right-4 top-4 p-3 rounded-xl bg-gradient-to-r from-purple-500 to-cyan-500 animate-astra-glow">
                <Sparkles className="h-5 w-5 text-white animate-astra-float" />
              </div>
            </div>
          </div>
          
          <Select value={selectedState} onValueChange={setSelectedState}>
            <SelectTrigger className="input-astra h-16 border-border/20 text-lg">
              <SelectValue placeholder={currentText.state} />
            </SelectTrigger>
            <SelectContent className="astra-card border-border/20">
          ...
          <Button 
            ...
            className="btn-astra h-16 text-xl font-bold group"
          >
        ...
        {/* ASTRA Advanced Filters */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-12">
          <Select value={listingType} onValueChange={setListingType}>
            <SelectTrigger className="input-astra h-14 border-border/20">
          ...
          <div className="flex items-center gap-3">
            {amenityOptions.map((amenity) => (
              <Button
                ...
                className="h-14 px-6 glass-astra border-border/20 hover:bg-purple-400/10 transition-all duration-300 group"
              >
        ...
        {/* ASTRA Trending Searches */}
        <div className="text-left">
          <div className="flex items-center gap-3 mb-6">
            <TrendingUp className="h-6 w-6 text-purple-400 animate-astra-glow" />
          ...
          <div className="flex flex-wrap gap-4">
            {trendingSearches.map((term, index) => (
              <Badge 
                key={index} 
                variant="secondary" 
                className="cursor-pointer glass-astra bg-gradient-to-r from-purple-400/10 to-cyan-400/10 hover:from-purple-400/20 hover:to-cyan-400/20 transition-all duration-300 border-border/20 px-6 py-3 text-base font-medium group"
