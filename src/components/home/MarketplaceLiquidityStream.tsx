...
interface StreamItem { id: string; type: 'sold' | 'new' | 'syndication' | 'vendor' | 'rental'; title: string; location: string; price: number; time: string; }
...
const MarketplaceLiquidityStream = () => {
  ...
  const { data: streamItems = [] } = useQuery({
    ...
    queryFn: async (): Promise<StreamItem[]> => {
      const { data } = await supabase.from('properties').select('id, title, location, city, price, created_at, status').not('title', 'is', null).order('updated_at', { ascending: false }).limit(12);
      const types: StreamItem['type'][] = ['new', 'sold', 'syndication', 'vendor', 'rental'];
      return (data || []).map((p: any, i: number) => ({ id: p.id, type: types[i % types.length], title: p.title, location: p.location || p.city || '', price: p.price || 0, time: p.created_at }));
    },
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4">
      <div className="flex items-center gap-3 mb-5">
        <div className="relative"><Activity className="h-4 w-4 text-intel-blue" /><span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-intel-success rounded-full animate-pulse" /></div>
        <h2 className="text-lg sm:text-xl font-bold text-foreground">{t('homeComponents.marketplaceLiquidityFeed')}</h2>
        <Badge className="text-[10px] h-5 bg-intel-success/10 text-intel-success border border-intel-success/20">● {t('homeComponents.live').toUpperCase()}</Badge>
      </div>
