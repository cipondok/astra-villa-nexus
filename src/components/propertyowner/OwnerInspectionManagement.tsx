...
import { ClipboardCheck, Plus, Camera, CheckCircle, AlertTriangle, Eye, Loader2 } from "lucide-react";
...
const OwnerInspectionManagement = () => {
  ...
  const [form, setForm] = useState({ bookingId: "", inspectionType: "check_in" as string, notes: "" });
  ...
  // Create inspection with default checklist
  const createMutation = useMutation({
    mutationFn: async () => {
      if (!user || !form.bookingId) throw new Error("Missing data");
    ...
    onSuccess: () => {
      ...
      setCreateOpen(false);
      setForm({ bookingId: "", inspectionType: "check_in", notes: "" });
    },
  ...
  if (detailId && activeInspection) {
    ...
    return (
      <div className="space-y-3">
        ...
        {activeInspection.status !== "completed" && allRated && (
          <Card className="p-3">
    ...
    <div className="space-y-3">
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogTrigger asChild>
          <Button size="sm" className="w-full text-xs"><Plus className="h-3.5 w-3.5 mr-1" /> Buat Inspeksi Baru</Button>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader><DialogTitle className="text-sm">Buat Inspeksi Properti</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <div>
            ...
            <Button size="sm" className="w-full text-xs" onClick={() => createMutation.mutate()} disabled={!form.bookingId || createMutation.isPending}>
              {createMutation.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" /> : <ClipboardCheck className="h-3.5 w-3.5 mr-1" />}
              Buat Inspeksi
            </Button>
          </div>
        </DialogContent>
      ...
      {isLoading ? (
        ...
        <Card className="p-6 text-center">
          ...
          <p className="text-xs font-medium">Belum ada inspeksi</p>
          <p className="text-[9px] text-muted-foreground">Buat inspeksi untuk check-in/check-out tenant</p>
        </Card>
      ) : (
