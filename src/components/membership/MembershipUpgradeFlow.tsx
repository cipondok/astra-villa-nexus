...
const UPGRADE_REQUIREMENTS: Record<MembershipLevel, UpgradeStep[]> = {
  ...
  developer: [
    ...
    },
    {
      ...
      description: 'Complete business or developer profile',
      icon: <Building2 className="h-5 w-5" />,
      isRequired: true,
    },
    {
      id: 'subscription',
      title: 'Subscribe to Developer',
      description: 'Premium subscription with AI analytics & more',
      icon: <CreditCard className="h-5 w-5" />,
      isRequired: true,
    },
    {
      id: 'activation',
      title: 'Instant Activation',
      description: 'Developer benefits activated immediately',
      icon: <Sparkles className="h-5 w-5" />,
      isRequired: true,
    },
  ],
  vip_investor: [
    ...
    {
      id: 'subscription',
      title: 'Subscribe to VIP Investor',
...
export const MembershipUpgradeFlow: React.FC<MembershipUpgradeFlowProps> = ({
  ...
  const getActionButton = (step: UpgradeStep) => {
    switch (step.id) {
      ...
      case 'business_docs': return 'Upload Documents';
      case 'subscription': return 'Subscribe Now';
      case 'business_profile': return 'Setup Business';
      case 'concierge': return 'Get Assigned';
      case 'review': return 'Submit for Review';
      case 'activation': return 'Continue';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm p-0 overflow-hidden">
          ...
          <DialogHeader>
            <div className="flex items-center gap-2.5">
              <div className={cn(
                ...
                "bg-background/50 border", targetConfig.borderColor
              )}>
                {targetConfig.icon}
              </div>
              <div>
                ...
                <p className="text-[10px] text-muted-foreground">{steps.length} langkah</p>
              </div>
            </div>
          </DialogHeader>
          <div className="mt-3">
              ...
              return (
                <React.Fragment key={step.id}>
                  ...
                  {index < steps.length - 1 && (
                    <div className={cn("flex-1 h-px mx-1", index < currentStep ? "bg-primary" : "bg-muted")} />
                  )}
                </React.Fragment>
              );
            })}
                ...
                <CardContent className="p-3">
                  <div className="flex items-start gap-3">
                    <div className={cn(
                      ...
                      targetConfig.bgColor, targetConfig.color
                    )}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 mb-0.5">
                        ...
                        {steps[currentStep].isRequired ? (
                          <Badge variant="destructive" className="text-[8px] px-1 py-0 h-4">Wajib</Badge>
                        ) : (
                          <Badge variant="secondary" className="text-[8px] px-1 py-0 h-4">Opsional</Badge>
                        )}
                      </div>
                      ...
                      {steps[currentStep].id === 'profile' && currentLevel === 'free' && (
                        <div className="mt-2 p-2 rounded-md bg-accent/10 border border-accent/30">
                          <div className="flex items-center gap-1.5 text-accent-foreground">
                            ...
                            <span className="text-[10px] font-medium">Complete your profile first</span>
                          </div>
                        </div>
                      )}
                    </div>
        ...
        <div className="px-4 py-3 flex items-center justify-between">
          <Button variant="ghost" size="sm" onClick={handlePrevious} disabled={currentStep === 0} className="h-7 text-[10px] gap-1">
            <ChevronLeft className="h-3 w-3" /> Back
          </Button>
          <div className="flex gap-1.5">
            <Button variant="ghost" size="sm" onClick={() => onOpenChange(false)} className="h-7 text-[10px]">Batal</Button>
            <Button size="sm" onClick={handleStartUpgrade} disabled={isProcessing} className="h-7 text-[10px] gap-1">
              {isProcessing ? (
                <><Loader2 className="h-3 w-3 animate-spin" />Loading...</>
              ) : (
                <>{steps[currentStep] && getActionButton(steps[currentStep])}<ArrowRight className="h-3 w-3" /></>
              )}
            </Button>
          </div>
        </div>
