...
const Window = ({ data, isHovered, onHover, onLeave, isDayTime }) => {
  ...
  useFrame((state) => {
    if (meshRef.current && meshRef.current.material && isHovered) {
      (meshRef.current.material as THREE.MeshStandardMaterial).emissive.setHex(0x222222);
  ...
  return (
    <group position={data.position as [number, number, number]} rotation={rotation}>
      {/* Window Frame */}
      <Box 
        ref={meshRef}
      ...
      {/* Window Glass */}
      <Box args={[data.size[0], data.size[1], 0.05]} position={[0, 0, 0.05]}>
      ...
      {isHovered && (
        <Box args={[data.size[0] + 0.2, data.size[1] + 0.2, 0.15]} position={[0, 0, -0.1]}>
...
const HouseStructure = ({ isDayTime }) => {
  return (
    <group>
      ...
      {/* Right Wall (partial for terrace) */}
...
const ThreeDShowcase = () => {
  ...
  const resetCamera = () => {
    // This would reset the camera position if we had a ref to OrbitControls
  };
