...
Deno.serve(async (req) => {
  ...
  const { data: run } = await supabase
    ...
    .insert({ worker_name: "deal_scanner", status: "running" })
    .select("id")
    .single();

  try {
    const { data, error } = await supabase.rpc("scan_deal_alerts", {
    ...
    return new Response(
      JSON.stringify({ success: true, worker: "deal_scanner", alerts_generated: rowsAffected, duration_ms: durationMs }),
