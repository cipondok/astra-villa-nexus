...
Deno.serve(async (req) => {
  ...
  const { data: run } = await supabase
    ...
    .insert({ worker_name: "portfolio_analyzer", status: "running" })
    .select("id")
    .single();

  try {
    const { data, error } = await supabase.rpc("compute_portfolio_snapshots");
    ...
    return new Response(
      JSON.stringify({ success: true, worker: "portfolio_analyzer", snapshots_created: rowsAffected, duration_ms: durationMs }),
