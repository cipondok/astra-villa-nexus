...
Deno.serve(async (req) => {
  ...
  const { data: run } = await supabase
    ...
    .insert({ worker_name: "opportunity_scoring", status: "running" })
    .select("id")
    .single();

  try {
    const { data, error } = await supabase.rpc("recalc_opportunity_scores", {
    ...
    return new Response(
      JSON.stringify({ success: true, worker: "opportunity_scoring", rows_affected: rowsAffected, duration_ms: durationMs }),
