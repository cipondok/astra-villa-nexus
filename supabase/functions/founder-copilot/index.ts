...
async function gatherFounderContext() {
  ...
  try {
    ...
    if (allocs?.length) {
      const topCity = allocs[0];
      capitalAllocSummary = `Top Capital Zone: ${topCity.city} (Efficiency ${topCity.capital_efficiency_score}/100, Priority ${topCity.allocation_priority_score}/100, Direction: ${topCity.recommended_capital_direction})`;
