-- Fix customer contact information security vulnerability (v2)
...
DECLARE
    pol record;
BEGIN
    -- Get all policies for the table and drop them
    FOR pol IN 
        SELECT schemaname, tablename, policyname 
        FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'property_survey_bookings'
    LOOP
...
CREATE POLICY "secure_admin_survey_bookings_select" 
...
CREATE POLICY "secure_property_owner_survey_bookings_select" 
ON public.property_survey_bookings 
FOR SELECT 
USING (
  EXISTS (
...
CREATE POLICY "secure_customer_own_survey_bookings_select" 
ON public.property_survey_bookings 
FOR SELECT 
USING (
  EXISTS (
...
-- 4. Only authenticated users can insert new bookings
CREATE POLICY "secure_survey_booking_insert" 
ON public.property_survey_bookings 
FOR INSERT 
WITH CHECK (auth.uid() IS NOT NULL);

-- 5. Only admins can update booking status
CREATE POLICY "secure_admin_survey_bookings_update" 
ON public.property_survey_bookings 
FOR UPDATE 
USING (
  EXISTS (
...
CREATE POLICY "secure_super_admin_survey_bookings_delete" 
ON public.property_survey_bookings 
FOR DELETE 
USING (check_super_admin_email());

-- Create secure functions for accessing customer data with masking
