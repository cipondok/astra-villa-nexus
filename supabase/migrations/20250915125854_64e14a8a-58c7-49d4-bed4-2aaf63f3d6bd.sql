-- Comprehensive security enhancement for BPJS healthcare data
...
-- Function to decrypt healthcare data (admin only)
...
-- Secure function to get BPJS verification status (admin only)
...
-- Secure function to log BPJS verification attempt
...
-- Enhanced audit function for healthcare data access
...
CREATE TRIGGER audit_healthcare_data_access_logs
  BEFORE INSERT OR UPDATE OR DELETE OR SELECT ON public.bpjs_verification_logs
  FOR EACH ROW
  EXECUTE FUNCTION public.audit_healthcare_data_access();

CREATE TRIGGER audit_healthcare_data_access_verifications
  BEFORE INSERT OR UPDATE OR DELETE OR SELECT ON public.bpjs_verifications
  FOR EACH ROW
  EXECUTE FUNCTION public.audit_healthcare_data_access();

-- Add documentation and compliance notes
COMMENT ON TABLE public.bpjs_verification_logs IS 'HEALTHCARE SENSITIVE: Contains encrypted BPJS verification data. All access logged for HIPAA compliance. Use secure functions only.';
COMMENT ON TABLE public.bpjs_verifications IS 'HEALTHCARE SENSITIVE: Contains encrypted BPJS verification status. All access logged for HIPAA compliance. Use secure functions only.';
