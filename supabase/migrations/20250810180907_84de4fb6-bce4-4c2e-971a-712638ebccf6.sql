-- Robust ASTRA Token Schema (idempotent)
-- Ensure tables exist with required columns before creating indexes
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'astra_token_balances_user_fk'
  ) THEN
    ALTER TABLE public.astra_token_balances
      ADD CONSTRAINT astra_token_balances_user_fk
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their own balances"
      ON public.astra_token_balances FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    ...
    CREATE POLICY "Admins can view all balances"
      ON public.astra_token_balances FOR SELECT USING (check_admin_access());
  END IF;
END $$;
...
-- Ensure required columns if table already existed
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.astra_token_transactions
      ADD CONSTRAINT astra_token_transactions_user_fk
      FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their own transactions"
      ON public.astra_token_transactions FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Admins can view all transactions"
      ON public.astra_token_transactions FOR SELECT USING (check_admin_access());
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.astra_daily_checkins
      ADD CONSTRAINT astra_daily_checkins_user_fk
      FOREIGN KEY (user_id) REFERENCES public.profiles(id) ON DELETE CASCADE;
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their own checkins"
      ON public.astra_daily_checkins FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Admins can view all checkins"
      ON public.astra_daily_checkins FOR SELECT USING (check_admin_access());
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Anyone can view reward config"
      ON public.astra_reward_config FOR SELECT USING (true);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Admins can manage reward config"
      ON public.astra_reward_config FOR ALL USING (check_admin_access()) WITH CHECK (check_admin_access());
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their own claims"
      ON public.astra_reward_claims FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Admins can view all claims"
      ON public.astra_reward_claims FOR SELECT USING (check_admin_access());
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their own referrals"
      ON public.astra_referrals FOR SELECT USING (auth.uid() = referrer_id OR auth.uid() = referred_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Admins can view all referrals"
      ON public.astra_referrals FOR SELECT USING (check_admin_access());
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    CREATE POLICY "Users can view their own transfers"
      ON public.astra_token_transfers FOR SELECT USING (auth.uid() = sender_id OR auth.uid() = recipient_id);
  END IF;
  IF NOT EXISTS (
    ...
    CREATE POLICY "Admins can view all transfers"
      ON public.astra_token_transfers FOR SELECT USING (check_admin_access());
  END IF;
END $$;
...
SELECT 'ASTRA token schema ensured' AS status;