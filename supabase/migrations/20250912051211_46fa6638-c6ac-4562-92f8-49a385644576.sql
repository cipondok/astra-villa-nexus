...
-- Now let's check if approved_service_names table exists and populate detailed services
INSERT INTO approved_service_names (service_name, category_id, subcategory_id, is_active, display_order)
SELECT 
  service_name,
  vmc.id as category_id,
  vsub.id as subcategory_id,
  true,
  service_order
FROM vendor_main_categories vmc
JOIN vendor_subcategories vsub ON vmc.id = vsub.main_category_id,
(VALUES 
...
WHERE vmc.name = services.main_name AND vsub.name = services.sub_name;