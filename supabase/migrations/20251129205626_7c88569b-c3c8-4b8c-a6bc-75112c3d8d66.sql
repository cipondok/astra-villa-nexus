-- Fix the create_application_admin_alert function to use allowed type values
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'property_owner_requests' THEN
    ...
    alert_type := 'property_listing';  -- Use allowed type
  ...
  ELSIF TG_TABLE_NAME = 'vendor_requests' THEN
    ...
    alert_type := 'vendor_request';  -- Use allowed type
  ...
  ELSIF TG_TABLE_NAME = 'agent_registration_requests' THEN
    user_name := NEW.full_name;
    alert_type := 'agent_request';  -- Use allowed type
  ...
  -- Insert admin alert with allowed type values
