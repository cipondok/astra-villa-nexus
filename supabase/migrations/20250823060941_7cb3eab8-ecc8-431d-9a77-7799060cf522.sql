...
DROP POLICY IF EXISTS "block_all_anonymous_access_vendor_profiles" ON public.vendor_business_profiles;

-- Create comprehensive security policies with unique names

-- Explicit deny for anonymous users
CREATE POLICY "deny_anon_vendor_profiles_access"
...
CREATE POLICY "vendor_view_own_profile_secure"
ON public.vendor_business_profiles
FOR SELECT
TO authenticated
USING (vendor_id = auth.uid());

CREATE POLICY "vendor_insert_own_profile_secure"
ON public.vendor_business_profiles
FOR INSERT
TO authenticated
WITH CHECK (vendor_id = auth.uid());

CREATE POLICY "vendor_update_own_profile_secure"
ON public.vendor_business_profiles
FOR UPDATE
TO authenticated
USING (vendor_id = auth.uid())
...
CREATE POLICY "vendor_delete_own_profile_secure"
ON public.vendor_business_profiles
FOR DELETE
TO authenticated
USING (vendor_id = auth.uid());
...
CREATE POLICY "admin_full_vendor_profiles_access"
ON public.vendor_business_profiles
FOR ALL
TO authenticated
