-- Create unified categories table with 4-level hierarchy (fixed duplicate slug issue)
...
-- Migrate existing data from the 4 separate tables using shorter IDs and unique slugs
...
SELECT 
  ...
  name,
  'l1-' || LOWER(REPLACE(REPLACE(name, ' ', '-'), '&', 'and')),
  type,
  1,
  NULL,
...
SELECT 
  ...
  vsc.name,
  'l2-' || LOWER(REPLACE(REPLACE(vsc.name, ' ', '-'), '&', 'and')) || '-' || ROW_NUMBER() OVER (ORDER BY display_order, name),
  'service',
  2,
...
SELECT 
  ...
  asn.service_name,
  'l3-' || LOWER(REPLACE(REPLACE(REPLACE(asn.service_name, ' ', '-'), '/', '-'), '&', 'and')) || '-' || ROW_NUMBER() OVER (ORDER BY asn.service_name),
...
SELECT 
  ...
  'l4-' || LOWER(REPLACE(REPLACE(COALESCE(vsc.implementation_type, vsc.name), ' ', '-'), '&', 'and')) || '-' || ROW_NUMBER() OVER (ORDER BY vsc.display_order, vsc.name),
  'service',
  4,
  snm.new_id,
  COALESCE(jsonb_build_object('icon', vsc.icon, 'description', vsc.description, 'original_id', vsc.id), '{}'),
...
('prod-materials', 'Building Materials', 'l1-building-materials', 'product', 1, NULL, '{"icon":"🧱","description":"Construction and building materials"}', 100, true),

-- Flooring (Sub Category)
('prod-floor', 'Flooring', 'l2-flooring', 'product', 2, 'prod-materials', '{"weight_kg_sqm":25,"installation_required":true}', 1, true),

-- Imported Marble (Product Variant)
('prod-marble', 'Imported Marble', 'l3-imported-marble', 'product', 3, 'prod-floor', '{"colors":["white","beige","gray"],"thickness_mm":[18,20,25],"finish":["polished","matte"]}', 1, true),

-- Marble Specifications (Attributes)
('marble-pol', 'Polished White Marble', 'l4-polished-white-marble', 'product', 4, 'prod-marble', '{"price_range":{"min":45,"max":75,"unit":"sq_ft"},"thickness_mm":20,"finish":"polished","color":"white"}', 1, true),
('marble-mat', 'Matte Beige Marble', 'l4-matte-beige-marble', 'product', 4, 'prod-marble', '{"price_range":{"min":55,"max":85,"unit":"sq_ft"},"thickness_mm":25,"finish":"matte","color":"beige"}', 2, true),

-- Roofing (Sub Category)
('prod-roof', 'Roofing', 'l2-roofing', 'product', 2, 'prod-materials', '{"weather_resistance":"high","installation_complexity":"medium"}', 2, true),

-- Eco-Friendly Tiles (Product Variant)
('prod-tiles', 'Eco-Friendly Tiles', 'l3-eco-friendly-tiles', 'product', 3, 'prod-roof', '{"sustainability_rating":"A+","types":["solar-reflective","waterproof"],"coverage_sqft_per_case":25}', 1, true),

-- Furniture (Main Category)
('prod-furniture', 'Furniture', 'l1-furniture', 'product', 1, NULL, '{"icon":"🪑","description":"Home and office furniture"}', 101, true),

-- Living Room (Sub Category)
('prod-living', 'Living Room', 'l2-living-room', 'product', 2, 'prod-furniture', '{"room_type":"living","style_categories":["modern","classic","contemporary"]}', 1, true),

-- Luxury Sofas (Product Variant)
('prod-sofas', 'Luxury Sofas', 'l3-luxury-sofas', 'product', 3, 'prod-living', '{"materials":["fabric","leather"],"customizable":true,"seating_capacity":[2,3,4,5]}', 1, true);
