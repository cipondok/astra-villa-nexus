-- Secure financial transaction tables with proper RLS policies
...
-- Create audit trigger for financial access
CREATE OR REPLACE FUNCTION public.audit_financial_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log all financial table operations
...
CREATE TRIGGER audit_payout_requests
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.payout_requests
  FOR EACH ROW
  EXECUTE FUNCTION public.audit_financial_access();
...
CREATE TRIGGER audit_vendor_astra_balances
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.vendor_astra_balances
  FOR EACH ROW
  EXECUTE FUNCTION public.audit_financial_access();