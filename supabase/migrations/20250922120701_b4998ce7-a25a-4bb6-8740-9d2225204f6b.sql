-- Secure financial transaction tables - final approach
-- Drop dependent policies first, then function, then recreate everything

-- Step 1: Drop dependent policies on vendor_astra_balances 
DROP POLICY IF EXISTS "Vendors view own balance only with logging" ON public.vendor_astra_balances;
...
-- Step 5: Recreate the log_financial_access function
...
-- Step 6: Recreate vendor_astra_balances policies with the fixed function
CREATE POLICY "Vendors view own balance only with logging"
ON public.vendor_astra_balances
FOR SELECT
USING (
  vendor_id = auth.uid() 
  AND log_financial_access('vendor_astra_balances', 'SELECT', auth.uid()) IS NOT NULL
);
...
USING (
  ...
  AND log_financial_access('vendor_astra_balances', 'SELECT_ALL', auth.uid()) IS NOT NULL
);

-- Step 7: Create policies for new financial tables
